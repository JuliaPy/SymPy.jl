...
Plots.convertToAnyVector(ex::Sym, d::Dict; kw...) = Any[ex], nothing

function Plots.compute_y(xs, ex::Sym)
    u = free_symbols(ex)[1]
    mapsubs(ex, u, xs)
end
...
function Plots.build_series_args{T<:Real,S<:Real}(plt::Plots.Plot, x::Plots.AVec{T}, y::Plots.AVec{S}, zf::Sym; kw...)
    ...
    Plots.build_series_args(plt, x, y, surface; kw...)  # passes it to the zmat version
end


# list of expressions
function Plots.build_series_args(plt::Plots.Plot, f::SymOrSyms, x; kw...)
    @assert !(typeof(x) <: Sym)  # otherwise we'd hit infinite recursion here
    Plots.build_series_args(plt, x, f; kw...)
end

# special handling... xmin/xmax with function(s)
function Plots.build_series_args(plt::Plots.Plot, f::SymOrSyms, xmin::Real, xmax::Real; kw...)
...
Plots.build_series_args{T<:Real}(plt::Plots.Plot, fx::Sym, fy::Sym, u::Plots.AVec{T}; kw...) =
    Plots.build_series_args(plt, mapSymOrSyms(fx, u), mapSymOrSyms(fy, u); kw...)
    

Plots.build_series_args{T<:Real}(plt::Plots.Plot, u::Plots.AVec{T}, fx::SymOrSyms, fy::SymOrSyms; kw...) =
    Plots.build_series_args(plt, mapSymOrSyms(fx, u), mapSymOrSyms(fy, u); kw...)


Plots.build_series_args(plt::Plots.Plot, fx::Sym, fy::Sym, umin::Real, umax::Real, numPoints::Int = 1000; kw...) =
    Plots.build_series_args(plt, fx, fy, linspace(umin, umax, numPoints); kw...)
