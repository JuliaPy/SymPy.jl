...
# evaluation p:i -> j
(p::SymPermutation)(i::Integer) = p.__x(i)
...
## Indexing into group
Base.getindex(Gp::SymPermutationGroup, i::Int) = PyCall.py"$(Gp.__x)[$(i-1)]"  # 1-base

elements(gp::SymPermutationGroup) = [a for a in gp.elements]

Base.length(G::SymPermutationGroup) =  PyCall.py"len($(G.__x))"
